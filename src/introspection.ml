...
type config =
  { schema_path: string
  ; path_type: string } [@@deriving yojson]
...
let of_file f =
    let chan = Pervasives.open_in f in
    let json = Yojson.Basic.from_channel chan in
    Yojson.Basic.Util.(json |> member "data" |> member "__schema" |> schema_of_yojson)

let schema_of_config config_path =
  try
    Yojson.Safe.from_file config_path
    |> config_of_yojson
    |> Result.map
      ~f:(fun config ->
          match String.lowercase config.path_type with
          | "file" -> of_file config.schema_path
          | "url" -> failwith "TODO: support url"
          | s -> failwith (sprintf "Invalid schema path type %s" s))
  with _exn -> Error "TODO: map_exn" 
